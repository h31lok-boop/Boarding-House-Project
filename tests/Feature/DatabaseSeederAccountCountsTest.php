...
test('the account seeder creates exactly one admin two owners and five tenants', function () {
    $this->seed(GeoBoardAccessSeeder::class);

    expect(User::query()->count())->toBe(8)
        ->and(User::query()->where('role', 'admin')->count())->toBe(1)
        ->and(User::query()->where('role', 'owner')->count())->toBe(2)
        ->and(User::query()->whereIn('role', ['user', 'tenant', 'student'])->count())->toBe(5)
        ->and(User::query()->where('role', 'owner')->whereHas('ownerProfile')->count())->toBe(2)
