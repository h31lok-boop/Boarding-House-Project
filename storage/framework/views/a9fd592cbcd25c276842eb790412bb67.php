...
<?php $component->withAttributes([]); ?>
  ...
  <div class="space-y-6">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        ...
        <form method="GET" class="flex items-center gap-2 text-sm">
          ...
          <a href="<?php echo e(route('admin.boarding-houses.index')); ?>" class="px-3 py-2 rounded-lg border hover:bg-[color:var(--surface-2)]">Reset</a>
        </form>
      </div>
          ...
          <tbody class="divide-y divide-gray-100">
            <?php $__empty_1 = true; $__currentLoopData = $houses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $house): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
              <tr class="hover:bg-[color:var(--surface-2)]">
                <td class="px-5 py-3 font-medium "><?php echo e($house->name); ?></td>
                <td class="px-5 py-3 ui-muted"><?php echo e($house->address); ?></td>
                <td class="px-5 py-3 ui-muted"><?php echo e($house->capacity); ?></td>
                <td class="px-5 py-3 ui-muted"><?php echo e($house->tenants_count); ?> / <?php echo e($house->capacity); ?></td>
                <td class="px-5 py-3">
                  <?php
                ...
                <td class="px-5 py-3 text-right">
                  <div class="flex justify-end items-center gap-2">
                    <button type="button"
                      ...
                      data-capacity="<?php echo e($house->capacity); ?>"
                      data-status="<?php echo e($house->is_active ? 'Active' : 'Inactive'); ?>"
                      data-description="<?php echo e(e($house->description ?? 'No description available.')); ?>">
                    ...
                    <a href="<?php echo e(route('admin.boarding-houses.edit', $house)); ?>" class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-amber-50 text-amber-600 hover:bg-amber-100" title="Edit">
            ...
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
              <tr>
                <td colspan="5" class="px-5 py-6 text-center ui-muted">No boarding houses yet.</td>
              </tr>
            <?php endif; ?>
          </tbody>
        </table>
  ...
  <div id="houseModal" class="fixed inset-0 bg-black/30 backdrop-blur-sm hidden items-center justify-center z-50">
    <div class="ui-surface rounded-md shadow-xl w-full max-w-[600px] mx-4 max-h-[90vh] overflow-y-auto">
      <div class="px-6 py-5 border-b ui-border flex items-center justify-between">
        <h3 class="text-xl font-semibold ">Boarding House Details</h3>
        <button id="closeHouseModal" class="ui-muted ui-muted text-xl" aria-label="Close">×</button>
      </div>
      <div class="px-6 py-6 space-y-4 text-sm">
        ...
        <div class="grid grid-cols-[auto,1fr] items-center gap-6">
          ...
          <span id="modalHouseName" class="font-semibold "></span>
        </div>
        <div class="grid grid-cols-[auto,1fr] items-center gap-6">
          <span class="ui-muted font-medium">Address</span>
          <span id="modalHouseAddress" class=""></span>
        </div>
        <div class="grid grid-cols-[auto,1fr] items-center gap-6">
          <span class="ui-muted font-medium">Capacity</span>
          <span id="modalHouseCapacity" class=""></span>
        </div>
        <div class="grid grid-cols-[auto,1fr] items-center gap-6">
          <span class="ui-muted font-medium">Status</span>
          <span id="modalHouseStatus" class=""></span>
        </div>
        <div class="grid grid-cols-[auto,1fr] items-start gap-6">
          <span class="ui-muted font-medium">Description</span>
          <p id="modalHouseDescription" class=" leading-relaxed"></p>
        </div>
      </div>
      <div class="px-6 py-5 border-t ui-border flex justify-end">
...
<?php endif; ?>
<?php /**PATH C:\Users\Aiza\Documents\lesson 1\Boarding-House-Project\resources\views/admin/boarding-houses/index.blade.php ENDPATH**/ ?>