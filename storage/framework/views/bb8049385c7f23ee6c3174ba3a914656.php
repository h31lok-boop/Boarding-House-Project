<nav class="w-64 bg-white border-r border-gray-200 flex flex-col justify-start gap-3">
    <?php
        $dashRoute = 'dashboard';
        $isOwner = false;
        $isAdmin = false;
        ...
        if (auth()->check()) {
            ...
            $isOwner = $role === 'owner';
            $isTenantNav = $role === 'tenant' || (method_exists($user, 'hasRole') && $user->hasRole('tenant'));
        ...
        <div class="px-6 pb-6 space-y-1">
            <?php $__currentLoopData = $navLinks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $link): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php if($isTenantNav && $loop->first): ?>
                    <form id="tenantSidebarSearchForm" method="GET" action="<?php echo e(route('tenant.dashboard.search')); ?>" class="relative py-1">
...
<?php endif; ?>
<?php /**PATH C:\Users\Hazel\Herd\final-project\resources\views\layouts\navigation.blade.php ENDPATH**/ ?>