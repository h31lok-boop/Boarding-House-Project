...
<x-admin.shell>
  <div class="ui-card p-4 mb-6">
    ...
    <p class="text-sm ui-muted mt-1">For geotagging, open Add or Edit then click the map to set coordinates.</p>
  </div>
  ...
  <div class="space-y-6">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        ...
        <form method="GET" class="flex items-center gap-2 text-sm">
          ...
          <a href="{{ route('admin.boarding-houses.index') }}" class="px-3 py-2 rounded-lg border hover:bg-[color:var(--surface-2)]">Reset</a>
        </form>
      </div>
          ...
          <tbody class="divide-y divide-gray-100">
            @forelse($houses as $house)
              <tr class="hover:bg-[color:var(--surface-2)]">
                <td class="px-5 py-3 font-medium ">{{ $house->name }}</td>
                <td class="px-5 py-3 ui-muted">{{ $house->address }}</td>
                <td class="px-5 py-3 ui-muted">{{ $house->capacity }}</td>
                <td class="px-5 py-3 ui-muted">{{ $house->tenants_count }} / {{ $house->capacity }}</td>
                <td class="px-5 py-3">
                  @php
