...
<x-admin.shell>
      ...
      <div class="ui-card p-6">
        ...
        <form method="POST" action="{{ route('admin.boarding-houses.update', $house) }}" class="space-y-5">
          ...
          <div>
            ...
            <input id="addressField" name="address" value="{{ old('address', $house->address) }}" class="w-full border rounded-lg px-3 py-2" required>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium mb-1">Latitude</label>
              <input id="latitudeField" type="number" step="0.0000001" name="latitude" value="{{ old('latitude', $house->latitude) }}" class="w-full border rounded-lg px-3 py-2">
            </div>
            <div>
              <label class="block text-sm font-medium mb-1">Longitude</label>
              <input id="longitudeField" type="number" step="0.0000001" name="longitude" value="{{ old('longitude', $house->longitude) }}" class="w-full border rounded-lg px-3 py-2">
            </div>
          </div>
          <div class="space-y-2">
            ...
            <div id="adminEditMap" class="w-full border rounded-lg" style="height: 320px;"></div>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium mb-1">Monthly Payment (PHP)</label>
              <input type="number" step="0.01" min="0" name="monthly_payment" value="{{ old('monthly_payment', $house->monthly_payment) }}" class="w-full border rounded-lg px-3 py-2">
            </div>
            <div>
              ...
              <select name="approval_status" class="w-full border rounded-lg px-3 py-2">
                <option value="approved" @selected(old('approval_status', $house->approval_status) === 'approved')>Approved</option>
                <option value="pending" @selected(old('approval_status', $house->approval_status) === 'pending')>Pending</option>
                <option value="rejected" @selected(old('approval_status', $house->approval_status) === 'rejected')>Rejected</option>
              </select>
            </div>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input name="contact_name" value="{{ old('contact_name', $house->contact_name) }}" class="w-full border rounded-lg px-3 py-2">
            </div>
            <div>
              <label class="block text-sm font-medium mb-1">Contact Phone</label>
              <input name="contact_phone" value="{{ old('contact_phone', $house->contact_phone) }}" class="w-full border rounded-lg px-3 py-2">
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <textarea name="house_rules" rows="3" class="w-full border rounded-lg px-3 py-2">{{ old('house_rules', $house->house_rules) }}</textarea>
          </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      const map = L.map('adminEditMap').setView([defaultLat, defaultLng], 14);
