        ...
        <div class="ui-card p-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('tenant.matches.index') }}" class="rounded-lg border ui-border px-4 py-2 text-sm">Back to Matches</a>
                    <a href="{{ route('tenant.matches.explain', $candidate) }}" class="rounded-lg border border-sky-200 bg-sky-50 px-4 py-2 text-sm font-medium text-sky-700 hover:bg-sky-100">Explain with DeepSeek</a>
                    <a href="{{ route('tenant.match-profile.edit') }}" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500">Update Profile</a>
                </div>
            </div>
        </div>
        ...
        <div class="grid gap-6 xl:grid-cols-[0.72fr,1.28fr]">
            <aside class="space-y-6">
                ...
                <div class="ui-card p-6">
                    ...
                    @if ($requestState['status'] === 'none')
                        <form method="POST" action="{{ route('tenant.matches.requests.store', $candidate) }}" class="mt-4 space-y-3">
                            @csrf
                            <div>
                    ...
                    @elseif ($requestState['status'] === 'pending' && $requestState['direction'] === 'incoming')
                        <div class="mt-4 space-y-3">
                            ...
                            @endif
                            <div class="flex gap-3">
                                <form method="POST" action="{{ route('tenant.match-requests.accept', $requestState['request']) }}">
                                    @csrf
                                ...
                                <form method="POST" action="{{ route('tenant.match-requests.decline', $requestState['request']) }}">
                                    @csrf
                    ...
                    @elseif ($requestState['status'] === 'pending' && $requestState['direction'] === 'outgoing')
                        <div class="mt-4 space-y-3">
                            ...
                            @if ($requestState['request']?->message)
                                <p class="rounded-lg border ui-border p-3 text-sm ui-muted">{{ $requestState['request']->message }}</p>
                            @endif
                            <form method="POST" action="{{ route('tenant.match-requests.cancel', $requestState['request']) }}">
                                @csrf
