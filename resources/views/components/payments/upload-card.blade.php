...
<section
    ...
    {{ $attributes->merge(['class' => 'rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900']) }}
>
    <div class="mb-4">
    ...
    @if ($loading)
        <div class="grid animate-pulse gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(240px,0.9fr)]">
            <div class="h-56 rounded-xl bg-slate-100 dark:bg-slate-800"></div>
            <div class="h-56 rounded-xl bg-slate-100 dark:bg-slate-800"></div>
        </div>
    @else
        <form method="POST" action="{{ $action }}" enctype="multipart/form-data" @submit="if (!canSubmit()) { $event.preventDefault() } else { submitting = true }" class="space-y-4">
            @csrf

            <div x-show="isCashPayment()" x-cloak class="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800 dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-200">
            ...
            <div class="grid gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(240px,0.85fr)]">
                <div
                    ...
                    class="flex min-h-52 flex-col items-center justify-center rounded-xl border-2 border-dashed p-5 text-center transition duration-200 hover:border-[#2563eb]/60 hover:bg-blue-50/40 dark:hover:bg-blue-400/5"
                >
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-[#2563eb] shadow-sm dark:bg-slate-900 dark:text-blue-300">
                    ...
                    <p class="mt-3 text-xs font-semibold text-slate-950 dark:text-white">Drag and drop your receipt here</p>
                    ...
                    <label for="{{ $inputId }}" class="mt-4 inline-flex cursor-pointer items-center justify-center gap-1.5 rounded-lg bg-[#2563eb] px-3 py-2 text-xs font-semibold text-white shadow-sm transition duration-200 hover:bg-[#1d4ed8] focus:outline-none focus:ring-2 focus:ring-[#2563eb]/30">
                    ...
                    <p class="mt-3 text-[11px] font-medium text-slate-400 dark:text-slate-500">Maximum file size: 5 MB</p>
                ...
                <div x-show="isCashPayment()" x-cloak class="flex min-h-52 flex-col justify-center rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-400/20 dark:bg-amber-400/10">
                    ...
                    <h3 class="mt-3 text-sm font-semibold text-amber-950 dark:text-amber-100">Cash payment selected</h3>
                    <p class="mt-1.5 text-xs leading-5 text-amber-800 dark:text-amber-200">No receipt upload is required for cash payments. Enter the amount, payment date, and any notes so the landlord can verify it in person.</p>
                </div>
                ...
                <div>
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-800/40">
                        <template x-if="file">
                            <div>
                                <div class="flex h-32 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
                                    <template x-if="previewUrl">
                                        <img :src="previewUrl" alt="Receipt preview" class="h-full w-full object-cover">
                                    </template>
                                    <template x-if="!previewUrl">
                                        <div class="text-center">
                                            <svg class="mx-auto h-9 w-9 text-[#2563eb]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
                                ...
                                <button type="button" @click="removeFile()" class="mt-3 inline-flex items-center justify-center rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
                                    Remove file
                                </button>
                            </div>
                        </template>
                        ...
                        <template x-if="!file && existing">
                            <div>
                                ...
                                <div class="mt-3">
                                    ...
                                    <span
                                        class="mt-3 inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold leading-none ring-1 ring-inset"
                                        :class="{
                        ...
                        <template x-if="!file && !existing">
                            <div class="flex h-44 items-center justify-center text-center">
                                <div>
                                    <div class="mx-auto flex h-9 w-9 items-center justify-center rounded-xl bg-white text-slate-500 shadow-sm dark:bg-slate-900 dark:text-slate-400">
                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625A3.375 3.375 0 0 0 16.125 8.25h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5A3.375 3.375 0 0 0 10.125 2.25H8.25A2.25 2.25 0 0 0 6 4.5v15A2.25 2.25 0 0 0 8.25 21.75h7.5A2.25 2.25 0 0 0 18 19.5v-2.25" />
                                        </svg>
                                    </div>
            ...
            <div class="grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-800/40 lg:grid-cols-[minmax(200px,0.7fr)_minmax(0,1.3fr)]">
                ...
                <div class="rounded-xl border border-blue-100 bg-white p-3 shadow-sm dark:border-blue-400/20 dark:bg-slate-900">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex min-w-0 items-center gap-2.5">
                            <div x-show="paymentMethod === 'GCash'" class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#2563eb] text-[10px] font-black text-white shadow-sm shadow-blue-500/20">
                                GCash
                            </div>
                            <div x-show="paymentMethod === 'Maya'" x-cloak class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-emerald-500 text-[10px] font-black text-white shadow-sm shadow-emerald-500/20">
                                Maya
                            </div>
                            <div x-show="paymentMethod === 'Bank Transfer'" x-cloak class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-900 text-white shadow-sm dark:bg-slate-700">
                            ...
                            <div x-show="paymentMethod === 'Cash Payment'" x-cloak class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-amber-500 text-white shadow-sm shadow-amber-500/20">
                            ...
                            <div class="min-w-0">
                                <p class="text-xs font-semibold text-slate-950 dark:text-white" x-text="paymentMethod"></p>
            ...
            <div class="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900 md:grid-cols-2">
                ...
                <label x-show="isCashPayment()" x-cloak class="text-xs font-medium text-slate-700 dark:text-slate-200 md:col-span-2">Notes <span class="font-normal text-slate-400">(Optional)</span>
                    <textarea name="notes" rows="2" :disabled="!isCashPayment()" class="mt-1 w-full rounded-lg border-slate-200 bg-white text-xs shadow-sm focus:border-[#2563eb] focus:ring-[#2563eb]/30 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100">{{ old('notes') }}</textarea>
                    @error('notes')<span class="mt-1 block text-xs font-semibold text-red-600 dark:text-red-300">{{ $message }}</span>@enderror
                </label>
            </div>

            <div class="flex justify-end">
                <button type="submit" :disabled="submitting || Boolean(fileError)" class="inline-flex items-center justify-center gap-1.5 rounded-lg bg-[#2563eb] px-4 py-2 text-xs font-semibold text-white shadow-sm transition duration-200 hover:bg-[#1d4ed8] focus:outline-none focus:ring-2 focus:ring-[#2563eb]/30 disabled:cursor-not-allowed disabled:opacity-60">
