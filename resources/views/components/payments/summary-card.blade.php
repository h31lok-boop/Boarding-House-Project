...
<section {{ $attributes->merge(['class' => 'rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm shadow-slate-200/50 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none']) }}>
    ...
    @else
        <div class="mt-3 space-y-2">
            @foreach ($items as $item)
                <div class="flex items-center justify-between gap-3 text-xs">
                    ...
                    <span class="font-semibold tabular-nums text-slate-900 dark:text-white">{{ $item['amount'] }}</span>
                </div>
            @endforeach
        </div>
        ...
        <div class="flex items-center justify-between gap-4 rounded-xl bg-slate-50/80 px-3 py-2.5 dark:bg-slate-800/40">
            <span class="text-xs font-semibold text-slate-600 dark:text-slate-200">Total Amount</span>
            <span class="text-base font-bold tabular-nums text-slate-950 dark:text-white">{{ $total }}</span>
        </div>

        @if ($bannerTitle || $bannerDescription)
            <div class="mt-3 flex items-start gap-2 rounded-xl border border-amber-200/70 bg-amber-50 px-3 py-2 text-amber-800 dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-200">
                ...
                <div class="min-w-0">
                    ...
                    @if ($bannerDescription)
                        <p class="mt-0.5 text-[11px] leading-4 text-amber-700 dark:text-amber-200/80">{{ $bannerDescription }}</p>
                    @endif
                </div>
            </div>
        @endif
