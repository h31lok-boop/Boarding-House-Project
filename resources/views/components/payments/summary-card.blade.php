...
<section {{ $attributes->merge(['class' => 'rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900']) }}>
    ...
    @else
        ...
        <div class="flex items-center justify-between gap-4">
            <span class="text-xs font-semibold text-slate-700 dark:text-slate-200">Total Amount</span>
            <span class="text-lg font-bold text-slate-950 dark:text-white">{{ $total }}</span>
        </div>

        @if ($bannerTitle || $bannerDescription)
            <div class="mt-4 flex items-start gap-2.5 rounded-xl border border-orange-200 bg-orange-50 px-3 py-2.5 text-orange-800 dark:border-orange-400/20 dark:bg-orange-400/10 dark:text-orange-200">
                ...
                <div class="min-w-0">
                    @if ($bannerTitle)
                        <p class="text-xs font-semibold">{{ $bannerTitle }}</p>
                    @endif
                    @if ($bannerDescription)
                        <p class="mt-1 text-xs leading-4 text-orange-700 dark:text-orange-200/80">{{ $bannerDescription }}</p>
                    @endif
                </div>
            </div>
        @endif
