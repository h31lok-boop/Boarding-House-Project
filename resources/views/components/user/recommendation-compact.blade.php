...
<article {{ $attributes->merge(['class' => 'group overflow-hidden rounded-xl border border-slate-200 bg-white p-2 shadow-sm transition duration-300 hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-lg hover:shadow-slate-200/60 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-blue-400/40 dark:hover:shadow-slate-950/30']) }}>
    <div class="flex gap-2.5">
        <div class="h-16 w-20 shrink-0 overflow-hidden rounded-lg bg-slate-100 dark:bg-slate-800">
            <img
        ...
        <div class="min-w-0 flex-1 py-0.5">
            <div class="flex items-start justify-between gap-2">
                <div class="min-w-0">
                    <h3 class="truncate text-xs font-semibold text-slate-950 dark:text-white">{{ $house['name'] }}</h3>
                    <p class="mt-0.5 truncate text-[11px] text-slate-500 dark:text-slate-400">{{ $house['location'] }}</p>
                </div>
            ...
            <div class="mt-2 flex items-center justify-between gap-3">
                <p class="text-xs font-bold text-slate-950 dark:text-white">{!! $house['price'] !!}</p>
                <a href="{{ $house['url'] }}" class="text-xs font-semibold text-blue-600 hover:text-blue-700 dark:text-blue-300 dark:hover:text-blue-200">View</a>
            </div>
        </div>
    </div>
</article>
