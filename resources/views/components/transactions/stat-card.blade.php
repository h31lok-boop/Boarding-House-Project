...
<article {{ $attributes->merge(['class' => 'bm-stat-card p-4 dark:border-slate-800 dark:bg-slate-900']) }}>
    <div class="flex items-center gap-3">
        ...
        <div class="min-w-0">
            <p class="truncate text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ $label }}</p>
            <p class="mt-1 truncate text-xl font-bold text-slate-950 dark:text-white">{{ $value }}</p>
            @if ($description)
                <p class="mt-1 truncate text-xs text-slate-500 dark:text-slate-400">{{ $description }}</p>
            @endif
        </div>
    </div>
</article>
