...
<x-user.shell>
    <div class="space-y-6">
        <div class="ui-card p-4">
            <div class="flex items-center justify-between gap-4 flex-wrap">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('user.recommendations') }}" class="px-4 py-2 rounded-lg border ui-border text-sm hover:bg-[color:var(--surface-2)]">Recommendations</a>
        ...
        @if($nearMe && $nearestHouse)
            <div class="ui-card p-4 border border-emerald-300">
                ...
                <a class="inline-block mt-2 text-sm text-indigo-600" href="{{ route('user.browse.show', $nearestHouse) }}">View Details</a>
            </div>
        @endif

        @if(($recommendedHouses ?? collect())->isNotEmpty())
            <div class="ui-card p-4">
                <div class="flex items-center justify-between gap-3 flex-wrap">
                    <div>
                        <h3 class="font-semibold">Recommended Boarding Houses</h3>
                        <p class="text-sm ui-muted">Ranked from your budget, preferred amenities, location, vacancy, and occupant compatibility.</p>
                    </div>
                    <a href="{{ route('user.profile') }}" class="px-3 py-2 rounded-lg border ui-border text-sm">Update Preferences</a>
                </div>

                <div class="mt-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    @foreach($recommendedHouses as $item)
                        @php
                            $house = $item['house'];
                        ...
                        @endphp
                        <article class="rounded-lg border ui-border p-4 space-y-3">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <div class="text-right">
                                    <p class="text-2xl font-bold text-indigo-700">{{ $recommendation['recommendation_percent'] }}%</p>
                                    <p class="text-xs ui-muted">fit</p>
                                </div>
                            </div>

                            <div class="text-sm space-y-1">
                                <p>Price: <span class="font-semibold">{{ $price !== null ? 'PHP '.number_format((float) $price, 2) : 'N/A' }}</span></p>
                                <p>Available Rooms: <span class="font-semibold">{{ $availableRooms }}</span></p>
                                <p>Amenities: <span class="font-semibold">{{ $house->amenities->pluck('name')->take(3)->join(', ') ?: 'None' }}</span></p>
                            </div>

                            <ul class="space-y-1 text-xs ui-muted">
                                @forelse(array_slice($recommendation['reasons'], 0, 3) as $reason)
                                    <li>{{ $reason }}</li>
                                @empty
                                    <li>Recommendation is based on available saved preferences.</li>
                                @endforelse
                            </ul>

                            <a href="{{ route('user.browse.show', $house) }}" class="inline-flex px-3 py-2 rounded-lg bg-indigo-600 text-white text-xs">View Details</a>
                        </article>
                    @endforeach
                </div>
            </div>
        ...
        <form id="browseFilterForm" method="GET" action="{{ route('user.browse') }}" class="ui-card p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-3">
            ...
            <div class="lg:col-span-4 flex items-center justify-end gap-2">
                <a href="{{ route('user.browse') }}" class="px-3 py-2 rounded-lg border ui-border text-sm">Reset</a>
        ...
        <form method="GET" action="{{ route('user.browse.compare') }}" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                @forelse($houses as $house)
                    ...
                    <div class="ui-card p-4 space-y-3">
                        ...
                        <div class="text-sm space-y-1">
                            @if(!empty($house->recommendation))
                                <p>Recommendation: <span class="font-semibold text-indigo-700">{{ $house->recommendation['recommendation_percent'] }}%</span></p>
                            @endif
                        ...
                        <div class="flex gap-2">
                            <a href="{{ route('user.browse.show', $house) }}" class="px-3 py-2 rounded-lg border ui-border text-xs">View Details</a>
                        </div>
                    </div>
                @empty
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const mapApiUrl = @json(route('map.user.listings'));
