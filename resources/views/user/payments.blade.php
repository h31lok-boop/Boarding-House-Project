...
@php
    ...
    $checkoutAvailable = (bool) ($confirmPayment['available'] ?? false);
    $amountDue = $checkoutAvailable
        ? (float) ($confirmPayment['amount'] ?? 0)
        : (float) data_get($nextPayment, 'amount', $summaryTotal ?? 0);
    $nextDueDate = $checkoutAvailable
        ? ($confirmPayment['due_date'] ?? null)
        : data_get($nextPayment, 'due_date');
@endphp
...
<div x-data="{ detailOpen: false, receiptOpen: false, checkoutSubmitting: false, selected: {} }" class="space-y-4">
    <x-user.page-header
        ...
        title="Payments"
        subtitle="View what is due, pay securely, and open your receipts."
    />
    ...
    <section data-tenant-payment-overview class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex min-w-0 items-center gap-3">
                <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-600 text-xs font-black text-white">PM</span>
                <div class="min-w-0">
                    <h2 class="text-sm font-bold text-slate-950 dark:text-white">PayMongo checkout</h2>
                    <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">Card, GCash, Maya, and QR Ph</p>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-x-6 gap-y-3 sm:justify-end">
                ...
                @if ($checkoutAvailable)
                    <button type="button" @click="receiptOpen = true" class="inline-flex h-10 items-center justify-center rounded-xl bg-blue-600 px-4 text-xs font-bold text-white shadow-sm transition hover:bg-blue-700 focus:outline-none focus-visible:ring-4 focus-visible:ring-blue-200">
                        Review &amp; pay
                    </button>
                @elseif ($paymentSchedule->isNotEmpty())
                    <span class="inline-flex rounded-full bg-amber-50 px-2.5 py-1 text-[11px] font-bold text-amber-700 ring-1 ring-inset ring-amber-200 dark:bg-amber-400/10 dark:text-amber-300 dark:ring-amber-400/20">
                        Owner setup required
                    </span>
                @else
                    <span class="inline-flex rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700 ring-1 ring-inset ring-emerald-200 dark:bg-emerald-400/10 dark:text-emerald-300 dark:ring-emerald-400/20">
                        Up to date
                    </span>
                @endif
            </div>
    ...
    @if ($receipts->isNotEmpty())
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="border-b border-slate-100 px-3.5 py-3 dark:border-slate-800">
                <h2 class="text-sm font-semibold text-slate-950 dark:text-white">Receipts</h2>
            </div>
            <div class="divide-y divide-slate-100 dark:divide-slate-800">
                @foreach ($receipts as $receipt)
                    ...
                    @endphp
                    <div
                        class="flex cursor-pointer flex-wrap items-center justify-between gap-3 px-3.5 py-3 transition hover:bg-blue-50/40 focus-within:bg-blue-50/40 dark:hover:bg-slate-800/60"
                        role="button"
                        tabindex="0"
                    ...
                    >
                        <div>
                            <p class="text-xs font-bold text-slate-900 dark:text-white">{{ $receiptPayload['number'] }}</p>
                            <p class="mt-0.5 text-[11px] text-slate-500">{{ $receiptPayload['date'] }} · {{ $receiptPayload['method'] }}</p>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="text-sm font-bold text-slate-900 dark:text-white">{{ $receiptPayload['amount'] }}</span>
                            @if ($receipt->status === \App\Models\PaymentReceipt::STATUS_APPROVED)
                                <a @click.stop href="{{ route('payment-receipts.print', $receipt) }}" target="_blank" class="text-xs font-bold text-blue-600 hover:text-blue-700">Preview / Print</a>
                            @else
                                <span class="text-xs font-semibold text-amber-600">Pending verification</span>
                            @endif
                        </div>
                    </div>
                @endforeach
