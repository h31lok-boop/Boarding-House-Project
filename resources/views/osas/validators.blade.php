                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach($validators as $v)
                        <tr class="hover:bg-slate-50">
                            <td class="px-4 py-3 font-semibold"><a href="{{ $r('osas.validators.show',$v->id) }}" class="text-indigo-600 hover:underline">{{ $v->name }}</a></td>
                            <td class="px-4 py-3 ui-muted">{{ $v->email }}</td>
                            ...
                            <td class="px-4 py-3 ui-muted">{{ $v->validationTasks()->count() }}</td>
