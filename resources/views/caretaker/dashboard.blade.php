...
{{-- Stats --}}
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                @foreach ($stats as $stat)
                    <a href="{{ $statLinks[$stat['label']] ?? $r('caretaker.dashboard') }}" class="ui-card p-4 flex items-center gap-3 hover:shadow-md transition">
                        <span class="text-2xl">{{ $stat['icon'] }}</span>
                        <div>
                            <p class="text-xs ui-muted uppercase">{{ $stat['label'] }}</p>
                ...
                <div class="ui-card p-5 space-y-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm ui-muted">{{ $currentBooking?->room?->name ?? $tenant['room'] }}</p>
                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @foreach ($history as $row)
                                    ...
                                    <tr class="hover:bg-slate-50">
                                        <td class="px-4 py-3 font-semibold"><a class="text-indigo-600 hover:underline" href="{{ $r('caretaker.tenants.index') }}">{{ $row['tenant'] }}</a></td>
                ...
                <div class="ui-card p-5 space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex gap-2 text-xs ui-muted"><span>This Month</span><span>•</span><span>{{ count($maintenance) }}</span></div>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                            <thead class="ui-surface-2 ui-muted uppercase text-xs">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @foreach ($maintenance as $row)
                                    <tr class="hover:bg-slate-50">
                                        ...
                                        <td class="px-4 py-3">
                                            <form method="POST" action="{{ $r('caretaker.maintenance.priority', ['id' => $row['id']]) }}">@csrf<button class="px-2 py-1 rounded-full text-xs font-semiboldbg-amber-50 text-amber-700 border border-amber-100">{{ $row['priority'] }}</button></form>
                                        </td>
                                        <td class="px-4 py-3 flex items-center gap-2">
                                            <form method="POST" action="{{ $r('caretaker.maintenance.resolve', ['id' => $row['id']]) }}">@csrf<button class="px-2 py-1 rounded-full text-xs font-semiboldbg-indigo-50 text-indigo-700 border border-indigo-100">{{ $row['status'] }}</button></form>
                        ...
                        <table class="min-w-full text-sm">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @foreach ($complaints as $row)
                                    <tr class="hover:bg-slate-50">
                                        <td class="px-4 py-3 font-semibold"><a class="text-indigo-600 hover:underline" href="{{ $r('caretaker.incidents.show', ['id' => $row['id']]) }}">#{{ $row['id'] }}</a></td>
                                        <td class="px-4 py-3 text-slate-700">{{ $row['room'] }}</td>
                                        <td class="px-4 py-3 text-slate-700">{{ $row['floor'] }}</td>
                        ...
                        <table class="min-w-full text-sm">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @foreach ($notices as $notice)
                                    ...
                                    <tr class="hover:bg-slate-50">
                                        <td class="px-4 py-3 font-semibold"><a class="text-indigo-600 hover:underline" href="{{ $r('caretaker.notices.index') }}">{{ $notice['title'] }}</a></td>
