    ...
    <div class="space-y-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex items-center gap-3">
                ...
                <span class="px-2 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-600 border border-slate-200">{{ $tenants->count() }}</span>
            </div>
                ...
                @foreach($tenants as $t)
                    ...
                    <div class="ui-card p-5 space-y-4">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-center gap-3">
                                <img src="https://i.pravatar.cc/96?u={{ $t['id'] }}" class="h-12 w-12 rounded-full object-cover shadow" alt="{{ $t['name'] }}">
                                <div>
                                    <p class="text-base font-semibold text-slate-900">{{ $t['name'] }}</p>
                                    @if($t['age_label'])
                                        <p class="text-xs ui-muted">{{ $t['age_label'] }}</p>
                                    @endif
                                </div>
                            </div>
                            <span class="px-3 py-1 rounded-full text-xs font-semibold {{ $pillClass }}">{{ $pillLabel }}</span>
                        </div>

                        <div class="flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium {{ $tone['bar'] }}">
                            <span class="h-2.5 w-2.5 rounded-full {{ $tone['dot'] }}"></span>
                            <span>{{ $t['rent_status_line'] }}</span>
                        </div>
                        ...
                        <div class="space-y-2 text-sm text-slate-700">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p class="font-medium">&#8369;{{ number_format($t['rent_amount']) }}</p>
                                </div>
                            </div>
                        </div>

                        <div class="grid grid-cols-2 gap-2 text-sm">
                            <a href="{{ route('caretaker.tenants.show', $t['id']) }}" class="px-3 py-2 rounded-lg bg-slate-100 text-slate-700 border border-slate-200 font-semibold text-center">View Profile</a>
                            @if($t['is_overdue'])
                                <form method="POST" action="{{ route('caretaker.notices.store') }}">
                                    @csrf
                                    <input type="hidden" name="title" value="Overdue Rent Notice - {{ $t['name'] }}">
                                    <input type="hidden" name="audience" value="{{ $t['name'] }}">
                                    <input type="hidden" name="body" value="Your rent of PHP {{ number_format($t['rent_amount']) }} for {{ $t['room'] }} is overdue ({{ $t['rent_status_line'] }}). Please settle it as soon as possible.">
                        ...
                        <div>
                            ...
                            @elseif($t['status'] === 'Checked-in')
                                <button class="w-full px-4 py-2 rounded-full bg-emerald-600 text-white text-sm font-semibold shadow">Checked-In</button>
                            @else
                                <a href="{{ route('caretaker.tenants.show', $t['id']) }}" class="w-full inline-flex justify-center px-4 py-2 rounded-full bg-slate-100 text-slate-700 border border-slate-200 text-sm font-semibold">View Profile</a>
                            @endif
                        </div>
                    </div>
                @endforeach
